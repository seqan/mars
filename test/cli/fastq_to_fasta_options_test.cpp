...
{
    cli_test_result result = execute_app("mars", "-v");
    std::string expected
    {
...
{
    cli_test_result result = execute_app("mars", data("in.fastq"));
    EXPECT_EQ(result.exit_code, 0);
...
{
    cli_test_result result = execute_app("mars", data("in.fastq"), "-v");
    EXPECT_EQ(result.exit_code, 0);
    EXPECT_EQ(result.out, "> seq1\nACGTTTGATTCGCG\n> seq2\nTCGGGGGATTCGCG\n");
...
{
    cli_test_result result = execute_app("mars", data("in.fastq"), "-o", "out.fasta");
